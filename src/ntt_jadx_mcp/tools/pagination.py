"""
NTT JADX MCP - Pagination Utilities (adapted from jadx-mcp-server)
...
logger = logging.getLogger("ntt-jadx-mcp.pagination")
logger.setLevel(logging.ERROR)

# Evita duplicar handlers en reloads/imports
...
class PaginationUtils:
    ...
    def create_page_based_tool(base_func: Callable) -> Callable:
        ...
        async def page_wrapper(
            page: int = 1,
            page_size: int = PaginationUtils.DEFAULT_PAGE_SIZE,
            **kwargs,
        ) -> dict:
            page = max(1, page)
